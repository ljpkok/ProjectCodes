...
parser.add_argument('--sampled_data_dir',
                    default=r'D:\BaiduYunDownload\UCF-101',
                    type=str, help='The dir for the sampled row data')
parser.add_argument('--ucf_list_dir', default=r'Data_UCF101/UCF101_video_list',
...
def main():
    ...
    folder_dir = set_project_folder_dir(args.open_new_folder, args.local_dir) ##默认使用当前目录
    print('The setting of the run are:\n{}\n'.format(args))
    print('The training would take place on {}\n'.format(device))
    print('The project directory is {}'.format(folder_dir))
    save_setting_info(args, device, folder_dir) ##存储运行参数
    ...
    plot_label_distribution(dataloaders, folder_dir, args.load_all_data_to_RAM, label_decoder_dict) ##绘制训练集/验证集分布
    print('Data prepared\nLoading model...')
